    ...
    <section className="space-y-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Rules are stored on backend and executed from webhook events.</p>
        </div>
        <button type="button" onClick={loadAutoReplyRules} className="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700">Refresh</button>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {isLoading ? <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-500">Loading rules...</td></tr> : null}
            {!isLoading && rules.length === 0 ? <tr><td colSpan={4} className="px-4 py-8 text-center text-sm text-gray-500">No auto-reply rules configured.</td></tr> : rules.map((rule) => (
              <tr key={rule.id}>
                ...
                <td className="max-w-md truncate px-4 py-3 text-gray-600" title={rule.replyMode === 'template' ? rule.templateName : rule.replyText}>{rule.replyMode === 'template' ? `Template: ${rule.templateName} · ${rule.templateLanguage}` : rule.replyText}</td>
                <td className="px-4 py-3"><button type="button" onClick={() => handleToggle(rule.id)} className={`rounded-full px-2.5 py-1 text-xs font-medium ${rule.active ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>{rule.active ? 'Active' : 'Inactive'}</button></td>
                <td className="px-4 py-3"><div className="flex justify-end gap-2"><button type="button" onClick={() => openEditModal(rule)} className="rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700">Edit</button><button type="button" onClick={() => handleDelete(rule.id)} className="rounded-md border border-red-200 px-3 py-1.5 text-xs font-medium text-red-700">Delete</button></div></td>
              </tr>
            ))}
          </tbody>
        </table>
